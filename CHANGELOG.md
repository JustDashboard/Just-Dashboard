...
- Files draws every folder and file as what it is, and folders take colours
  - Folders are drawn as folders, with what they hold pressed into them: git, Docker, Node, Go, Python and other products by their marks, keys, settings, logs and backups by theirs. Files are pages carrying their format's own logo and a band in its colour with the extension on it. Right-click a folder, use the swatches beside it or the folder in the top strip to give it one of nine colours; the colour is kept on the server, follows the folder when it is renamed or moved, and shows in the listing, the sidebar, the finder and the terminal's file tree. The page header is gone: Find, New, Upload and the GitHub account sit in the strip above the listing with the path, the sidebar has no header of its own and draws / as the host's distribution, and the details column shows the folder you are in when nothing is selected.
...
- Every deployment page draws what it is about, and a project's settings read as forms
  - The projects page opens on what is live, how much traffic there is and how much of it fails, and how busy the builders are, with the projects that need you first; each card shows the project's own logo, where its code comes from, its last commit, its hour of traffic and its last fourteen runs. A project opens on one line describing it and its site, a run shows who started it and the stage it has reached, and the build log groups each step, counts its errors and can be downloaded. The Logs page's readings each carry their last hour, and the first alert can be added right there. Every settings page puts its section names down the left and a Save at the foot of each form, and saving one section no longer throws away unsaved edits in another. Credentials are drawn as the hosts they sign in to, and notification channels and webhooks as the services they reach with their last fourteen deliveries; a pasted token names the service it belongs to, a .env import shows what each line will do before anything is written, Add domain shows the DNS record to create and whether it already points here, schedules show their next five runs, and deleting a project for good asks for its name. Restart and Stop ask before they interrupt the site, and an archived project can be restored from its Danger zone. Along the way a long traffic window says how wide its points really are and keeps the minute happening now, a project's links to its backup job and database open them, and a build whose commit could not be described keeps the rest of that step's log.

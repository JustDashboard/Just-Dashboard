...
| `/deploy/[id]/logs`, `/runtime`, `/console` | Four traffic readings over one pane of three views — Requests (what the ingress served, from a record the server keeps in memory), Output (the container's own lines) and Events (Docker's exits, OOM kills, restarts and health flips), see [`request-observability.md`](request-observability.md); services, live usage and recorded charts, routes/storage/backup evidence; a shell inside the live container. Game servers add `/players` and `/game-settings`. |

...
`uv.lock`, `poetry.lock`, `pdm.lock`, `Pipfile(.lock)`, `setup.cfg`, `environment.yml`, `go.mod`, `go.sum`, `go.work`, `Cargo.toml`, `Cargo.lock`, `.cargo/config.toml`, `pom.xml`, `build.gradle(.kts)`, `*.csproj`, `deno.json(c)`, a
...
`build_output_cause_compiled_test.go`. Persistent state, schema tools, seeds and their

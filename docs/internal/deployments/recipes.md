...
`NuGet.config` and `global.json` that apply to them, `deno.json(c)` and `deno.lock` with Deno's version
files, `composer.json` and `composer.lock`, a `Procfile`, the JDK and .NET SDK
...
text for literals), `Gemfile`/`Gemfile.lock`/`.ruby-version`, `mix.exs`/`mix.lock`, `build.sbt` and
`project/*.sbt`, `project.clj`, `deps.edn`/`build.clj`, `pubspec.yaml`/`pubspec.lock`,
`gleam.toml`/`manifest.toml`, and the site generators' configuration (`hugo.toml`, Zola's `config.toml`,
`book.toml`, Jekyll's `_config.yml` and `Gemfile`, `mkdocs.yml`, Sphinx's `conf.py`, `pelicanconf.py`,
Lume's `_config.ts`) — and names a candidate per root with the framework, the build and start commands, the port,
...
which preflight's `recipe_unsupported` shows instead of "No deployable plan was detected": Middleman;
Crystal, Haskell, Zig, Swift, OCaml, Nim, Perl, Erlang, R Shiny and Plumber, C/C++ (CMake, Meson), Elm
and a Flutter web app. The same pass recognises Ruby, Elixir, Scala, Clojure, Dart and Gleam, which have
recipes of their own ([Ruby](#ruby), [Elixir](#elixir), [Scala and Clojure](#scala-and-clojure),
[Dart and Gleam](#dart-and-gleam)): their candidate is built from the recipe's own reading of the root
(`detect_languages.go`), beside a Dockerfile when the root has one. Hugo, Zola, mdBook, Jekyll, MkDocs,
Sphinx, Pelican and Lume have recipes too ([Static sites](#static-sites-and-site-generators)), and F#
builds with the .NET recipe. A web framework among the manifest's dependencies makes it a web service on
its conventional port. A Rails, Hanami or Phoenix application owns its `package.json` (and
`assets/package.json`): that asset pipeline is set aside rather than offered as a Node service, and the
Ruby or Elixir recipe installs and builds it — a CocoaPods or fastlane Gemfile beside a React Native app
owns nothing. A site generator (Middleman, Elm, and the generators with a recipe) owns a tooling-only
`package.json` at its root the same way: a Hugo site with a Tailwind build used to be only a Node worker
asking for a start command. A Flutter app's `web/` folder is a template `flutter build web` fills in,
never a site: served raw it was a blank page that passed readiness, so it is set aside as the app's own
files and the app is named with its build. There is no Flutter recipe: Flutter publishes no official
image, and its SDK and packages are fetched from hosts a build cannot be proven against here.
...
other language is named only when nothing else at its root is a candidate, and C/C++ never inside
another candidate's root, where it is that application's vendored code. Each entry is removed when its
recipe lands.
...
Packaged static output — a JavaScript or site generator's build, a Python or Deno site, a Trunk site, a
Blazor WebAssembly app — always serves on nginx port 80, regardless of a repository's development/start
...
(`uv sync --frozen --all-groups`, `poetry install --all-groups`, on the uv and Poetry releases the Python
recipe pins). A site builds on the Python recipe's own image, interpreter choice and install planner
(`selectPythonSiteRecipe`): the same `python:<family>-slim-trixie`, the family the plan or the source's
version files, pyproject and wheels choose, and the Debian packages its dependencies or
`build.systemPackages` name. Plugins and extensions are read from the
...
`project/`, a Gemfile or `manifest.toml`, and a site generator's configuration: `hugo.toml`, Zola's
...
wherever they offer the package manager: a JavaScript recipe, and the Node install of a PHP or Python
asset stage and of a Ruby or Elixir build, `nodeInstallRecipes`) outranks everything the repository
...
build command that writes files — on the same interpreter image, system packages and install described
below, and nginx serves what it wrote ([Static sites](#static-sites-and-site-generators)). A
documentation tool the application install leaves out, in a uv or Poetry dependency group, is installed
with every group on the same pinned uv or Poetry. `dependencies_unpinned` names each recipe's own way to
pin: `composer.lock` for PHP, `deno.lock` for Deno, `Cargo.lock` for Rust, `Gemfile.lock` for Ruby and
Jekyll, `mix.lock` for Elixir, `pubspec.lock` for Dart, `manifest.toml` for Gleam, a lock or a frozen
requirement file for a Python application, and a requirements file for a Python site that declared none.
...
of its own, and `build.packageManager` chooses its manager among competing lockfiles, as `build.nodeVersion`
chooses its Node major.
...
- **Trunk** (a `Trunk.toml`, or an `index.html` with `data-trunk` links, beside yew, leptos, dioxus-web,
  ...
  --release --dist /out/dist`, served by the static server every static output uses
  ([JavaScript and static output](#javascript-and-static-output)) with the single-page fallback always
  on. The `index.html` is its source, so it is not a static-site candidate of its own.
...
| blazor-wasm | `Microsoft.NET.Sdk.BlazorWebAssembly` | published, its `wwwroot` served by the static server every static output uses, with the plan's single-page fallback (static, port 80) |
...
| `build_registry_auth`, `build_registry_rate_limited`, `build_network` | E401/E403, `YN0041`, `terminal prompts disabled` (from the go command's git fetch: a private module without `GIT_TOKEN`), npm's `Failed to replace env in config: ${X}` (naming the variable `.npmrc` authenticates with), Composer's `URL required authentication` (whose remedy is `COMPOSER_AUTH` for the install), NuGet `NU1301` with 401/403, a Maven or Gradle repository's `status code: 401` (read before the dependency it could not resolve); `toomanyrequests`; DNS, TLS and connection failures | — |
...
Leiningen uberjar on Ring and a Gleam Mist server, and the site generators: Hugo (Sass through Hugo Pipes,
the build value read with `getenv`), Zola, mdBook (two renderers, the title from `MDBOOK_BOOK__TITLE`),
Jekyll 4 from its `Gemfile.lock` (a plugin reading the build value), MkDocs with Material, Lume and
Eleventy with no build script and its output in its configuration, each also fetched by a clean URL and
answering a missing page with the site's own `404.html`; the SvelteKit static fixture serves a
prerendered `/about` as `about.html` and falls back to its `200.html`. The JVM and .NET

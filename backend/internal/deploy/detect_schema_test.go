...
func TestPythonAndDotnetSchemaToolsApplyTheSchemaBeforeServing(t *testing.T) {
	...
	for _, fixture := range []struct {
		...
		},
		{
			...
			start: "flask --app app db upgrade && gunicorn --bind 0.0.0.0:8000 app:app",
		},
		{
			...
			start: "aerich upgrade && uvicorn main:app --host 0.0.0.0 --port 8000",
		},
		{
			...
			start: "python manage.py migrate --noinput && gunicorn mysite.wsgi:application --bind 0.0.0.0:8000",
		},
		{

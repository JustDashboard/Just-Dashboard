...
func buildEnvValidationFindings(candidate *DetectedCandidate, configuration PlanConfiguration, values map[string]string) []PreflightFinding {
	...
	for _, name := range record.EnvServer {
		if envProvidedNames[name] || buildValue(configuration, values, name) {
			continue
		}
	...
	for _, name := range record.EnvClient {
		if !buildValue(configuration, values, name) {
...
func platformVariableFindings(configuration PlanConfiguration, values map[string]string) []PreflightFinding {
	...
	for _, name := range []string{"HOST", "HOSTNAME"} {
		if !served || !scoped(name, "runtime") {
			continue
		}
		if host := strings.TrimSpace(values[name]); nodeLoopbackHost(host) {
			findings = append(findings, finding("host_variable_loopback", PreflightWarning,
				name+" binds the server to loopback", name+"="+host,
				"Servers that bind to "+name+" (Next.js standalone, Nuxt, Astro, SvelteKit, …) then listen only inside the container, where the proxy and the readiness check cannot reach them.",
				"Remove "+name+", or set it to 0.0.0.0.", "deploy", "variables."+name))
		}
	}
	return findings
}

...
func elixirCandidate(buildRoot, root string, match *ecosystemMatch) DetectedCandidate {
	...
	if project.assets {
		if assets, err := planLanguageNodeAssets(buildRoot, "assets", "", nodeTargetArch("")); err == nil {
			...
			candidate.NodeInstalls = facts.detectedInstalls(assets.plan.manager, true, func(runner string) (string, string) { return runner + " run build", "" })
...
func selectElixirRecipe(root string, config BuildPlanConfig) (elixirRecipe, error) {
	...
	if project.assets {
		assets, err := planLanguageNodeAssets(root, "assets", config.PackageManager, nodeTargetArch(config.TargetPlatform))
		if err != nil {

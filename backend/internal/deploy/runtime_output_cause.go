...
// checks failed. It carries identifiers and the objects they name, never a
...
type OutputCause struct {
	...
	// module, a migration. Detail names the tool the code is about.
...
var runtimeSignatures = []buildSignature{
	...
	signature("runtime_database_localhost", "", "", "ECONNREFUSED (?:127\\.0\\.0\\.1|::1|localhost):(\\d+)|Can't reach database server at `(?:localhost|127\\.0\\.0\\.1):(\\d+)`|connection to server at \"localhost\" \\([^)]*\\), port (\\d+) failed|(?:localhost|127\\.0\\.0\\.1):(\\d+)\\D*Connection refused"),
	...
	signature("runtime_schema_push_refused", "", "", `--accept-data-loss|Do you still want to push changes|data loss`),
}
...
func applicationOutputCause(containers []ContainerDiagnostics, context runtimeCauseContext) *OutputCause {
	...
	for _, line := range lines {
		if match := loopbackListenRE.FindStringSubmatch(line.text); match != nil && !anyLineMatches(lines, runningOnAllAddressesRE) {
			cause := &OutputCause{Code: "runtime_loopback_bind", Subjects: addFirstGroup(nil, match)}
...
func (c *OutputCause) sentence() string {
	...
	case "runtime_origin_rejected":
		return "Phoenix rejected the socket's origin; set `check_origin` or the endpoint's host to the deployment's hostname"
	...
	case "runtime_schema_push_refused":
		return "the schema push refused a change that would lose data; commit migrations and apply them with `migrate deploy` instead of a push"
	case "runtime_loopback_bind":
		if c.Fix != nil {
			return "the application listens on localhost only, which nothing outside its container can reach; start it with `" + c.Fix.Value + "`"
		}
		return "the application listens on localhost only, which nothing outside its container can reach; bind it to 0.0.0.0"
	...
	case "runtime_start_exited":
		return "the start command exited instead of serving; start the server in the foreground (for pm2, `pm2-runtime`)"
	}
	return ""
}
...
func (c *OutputCause) releaseSentence() string {
	...
	case "release_database_unreachable":
		return "the task could not reach the database" + parenthesized(subject) + "; release tasks run on the dashboard's own host, where a project network address such as db-N.jd.internal does not resolve — run the migration from the start command instead"
	...
	case "release_task_command_not_found":
		return "`" + orDefault(subject, "the command") + "` is not installed where release tasks run: they run in the dashboard's own shell over the unbuilt source, without the application's runtime or dependencies; run it from the start command instead, for example `npx prisma migrate deploy && npm run start`"
	}
	return ""
}

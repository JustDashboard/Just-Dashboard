...
func TestPreflightNamesComposeBuildArgumentsThatCannotBePassed(t *testing.T) {
	...
	secret, ok := findingWithCode(findings, "compose_build_arg_secret")
	...
	if unscoped, ok := findingWithCode(findings, "compose_build_arg_unscoped"); !ok || unscoped.Severity != PreflightWarning ||
	...
	if item, ok := findingWithCode(composeBuildArgFindings(&required, PlanConfiguration{Variables: []PlannedVariable{
...
func TestOperatorChoosesTheComposePrimaryService(t *testing.T) {
	...
	if item, ok := findingWithCode(findings, "compose_primary_service_missing"); !ok || item.Severity != PreflightBlocked {
	...
	if item, ok := findingWithCode(findings, "compose_primary_service"); !ok || item.Measured != "db" {

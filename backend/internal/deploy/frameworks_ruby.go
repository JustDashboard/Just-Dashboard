...
func rubyCandidate(buildRoot, root string, match *ecosystemMatch) DetectedCandidate {
	...
	if install, _ := project.needsNode(); install {
		...
		if assets, err := planLanguageNodeAssets(buildRoot, "", "", nodeTargetArch("")); err == nil {
...
func selectRubyRecipe(root string, config BuildPlanConfig) (rubyRecipe, error) {
	...
	switch {
	case install:
		assets, err := planLanguageNodeAssets(root, "", config.PackageManager, nodeTargetArch(config.TargetPlatform))
		if err != nil {

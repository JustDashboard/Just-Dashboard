...
`
	...
	cause := runtimeOutputCause([]ContainerDiagnostics{{State: "running", Listening: sockets}})
	if cause == nil || cause.Code != "loopback_only" || cause.Listener != "127.0.0.1:3000" ||
		!strings.Contains(cause.sentence(), "listening only on 127.0.0.1:3000") {
	...
	if len(open) != 2 || open[0].String() != "0.0.0.0:8080" || listenerCause([]ContainerDiagnostics{{Listening: open}}) != nil {
		t.Fatalf("open = %+v", open)
	}
	// What the output proves comes first.
	schema := runtimeOutputCause([]ContainerDiagnostics{{
	...
	if !strings.Contains(suffix, "listening only on 127.0.0.1:3000") || strings.Contains(suffix, "build log") {

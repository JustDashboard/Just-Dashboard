...
func TestRecipesRenderFrameworkEnvironmentsEntriesAndFallbacks(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{"FROM python:3.12-slim-trixie@sha256:", "ENV PYTHONUNBUFFERED=1", "pip install --no-cache-dir --requirement requirements.txt",
		...
		},
		{
			...
			absent: []string{"uvicorn[standard]==0.35.0"},
		},
		{
			...
			files: map[string]string{"pyproject.toml": "[project]\nname = \"x\"\nrequires-python = \">=3.11\"\ndependencies = [\"fastapi\"]\n",
				"uv.lock": "[[package]]\nname = \"fastapi\"\n", "main.py": "app = FastAPI()\n"},
			config: BuildPlanConfig{Method: BuildRecipe, Recipe: "python", StartCommand: "uvicorn main:app --host 0.0.0.0 --port 8000"},
			want: []string{"ENV VIRTUAL_ENV=/app/.venv", "ENV PATH=/app/.venv/bin:$PATH", "ENV UV_PYTHON_DOWNLOADS=never",
				"pip install --no-cache-dir uv==" + pythonUVRelease + " && uv sync --locked --no-dev --python /usr/local/bin/python", "RUN uv pip install --no-cache 'uvicorn[standard]==0.35.0'"},
		},
		{
		...
		},
		{
			...
			want:   []string{"FROM python:3.10-slim-trixie@sha256:", "pip install --no-cache-dir tomli==2.0.1", "import tomli as tomllib", "ENV GRADIO_SERVER_NAME=0.0.0.0"},
		},
		{
			...
			want:   []string{"FROM python:3.11-slim-trixie@sha256:"},
		},
		{

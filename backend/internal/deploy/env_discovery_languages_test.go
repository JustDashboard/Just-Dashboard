...
func TestPythonReadHasDefault(t *testing.T) {
	...
	for _, test := range []struct {
		...
		if got := pythonReadHasDefault(test.callee, callArgumentList(content, 0)); got != test.want {
			t.Fatalf("%s(X, %s = %v, want %v (arguments %q)", test.callee, test.arguments, got, test.want, callArgumentList(content, 0))
		}
	}
}

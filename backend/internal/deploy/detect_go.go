...
func applyGoBuildFacts(checkout string, candidate *DetectedCandidate, packages goModulePackages, marker *detectedMarkers, candidates []DetectedCandidate, sums *int64) {
	...
	*sums -= plan.sumBytes
	facts := &DetectedGoBuild{
		...
		CGOUnknown: boundedList(plan.cgo.unknown), Vendored: plan.vendored, SumMissing: plan.sumAbsent,
		SumStale: boundedList(plan.sumStale), OwnerModules: boundedList(plan.ownerModules), Codegen: plan.templ,
	}
	if plan.cgo.enabled() {
...
// boundedList keeps the first 64 entries of a list a candidate records.

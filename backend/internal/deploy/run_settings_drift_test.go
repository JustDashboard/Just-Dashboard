...
func TestRunSettingsDriftNamesWhatChangedSinceTheRun(t *testing.T) {
	...
	fixture.addPlanWithRuntime(t, 2, strings.Repeat("a", 40), RuntimePlanConfig{
		...
		Command: []string{}, Capabilities: []string{}, Devices: []string{}, Mounts: []RuntimeMount{{Source: "shop-data", Target: "/data"}},
	})
	// It also names the Dockerfile's stage and moves the migration into the
	// release image, and gives the state a volume.
	build, _ := json.Marshal(BuildPlanConfig{Method: BuildDockerfile, Dockerfile: "Dockerfile", PackageManager: "bun", Target: "production",
		Secrets: []BuildSecretConfig{}, ReleaseTasks: []ReleaseTaskConfig{{Name: "migrate", Command: "npx prisma migrate deploy", Runner: ReleaseTaskRunnerImage}}})
	...
	want := []SettingsChange{
		...
		{Kind: "build", Field: "target", Change: "added", After: "production"},
		{Kind: "build", Field: "releaseTasks", Change: "added", After: "migrate=npx prisma migrate deploy (image)"},
		{Kind: "runtime", Field: "internalPort", Change: "changed", Before: "3000", After: "8080"},
		{Kind: "runtime", Field: "mounts", Change: "added", After: "/data"},

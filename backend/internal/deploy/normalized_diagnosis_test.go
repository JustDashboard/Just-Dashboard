...
func TestReleaseTaskFailureNamesTimeoutsAndCauses(t *testing.T) {
	...
	// In the release image the program is the image's to have, and the
	// database's network is joined, so neither sentence sends the task there.
	imageTask := task
	imageTask.Runner = ReleaseTaskRunnerImage
	code, message, cause = releaseTaskFailure(imageTask, ReleaseTaskEvidence{ExitCode: 127}, errors.New("release task exited with code 127"),
		[]collectedLine{{text: "/bin/sh: npx: not found", seq: 9}}, nil)
	if code != "release_task_command_not_found" || cause.Detail != "release image" || !strings.Contains(message, "not installed in the release image") {
		t.Fatalf("image task = %s %q", code, message)
	}
	_, message, _ = releaseTaskFailure(imageTask, ReleaseTaskEvidence{ExitCode: 1}, errors.New("release task exited with code 1"),
		[]collectedLine{{text: "Error: P1001: Can't reach database server at `db-4.jd.internal:5432`"}}, nil)
	if !strings.Contains(message, "joins the project's database networks") {
		t.Fatalf("image task unreachable = %q", message)
	}
	// Exit status 127 without the shell's line still names the missing program.
	if code, _, cause = releaseTaskFailure(task, ReleaseTaskEvidence{ExitCode: 127}, errors.New("release task exited with code 127"), nil, nil); code != "release_task_command_not_found" || cause == nil {
		t.Fatalf("bare 127 = %s %+v", code, cause)
	}

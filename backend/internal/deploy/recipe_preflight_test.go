...
// What the JavaScript install planner builds anyway is not a refusal: a
// repository without a lockfile installs unfrozen, and bun.lock supersedes a
// bun.lockb beside it. The dry run prepares them, and preflight says what a
// rebuild may do differently instead.
func TestInstallShapesThePlannerBuildsAreNotRefused(t *testing.T) {
	next := `{"name":"web","scripts":{"build":"next build","start":"next start"},"dependencies":{"next":"16.0.0"}}`
	laravel := `{"require":{"php":"^8.3","laravel/framework":"^12.0"}}`
	assets := `{"scripts":{"build":"vite build"},"devDependencies":{"laravel-vite-plugin":"1.0.0","vite":"6.0.0"}}`
	for _, fixture := range []struct {
		...
		{"no lockfile", map[string]string{"package.json": next},
			BuildPlanConfig{Method: BuildRecipe, Recipe: "node", BuildCommand: "npm run build", StartCommand: "npm run start"}, "dependencies_unpinned"},
		...
		{"Laravel assets without a lockfile", map[string]string{"composer.json": laravel, "composer.lock": "{}", "package.json": assets},
			BuildPlanConfig{Method: BuildRecipe, Recipe: "php", StartCommand: "frankenphp php-server --root public/"}, "assets_dependencies_unpinned"},
	} {
		t.Run(fixture.name, func(t *testing.T) {
			root := t.TempDir()
			for path, content := range fixture.files {
			...
			if err := dryRunBuild(t.Context(), root, fixture.build, nil); err != nil {
				t.Fatalf("dry run refused: %v", err)
			}
			findings := draftTreeFindings(t, root, fixture.build, &treeInspectorFake{root: root, head: strings.Repeat("a", 40)})
			for _, item := range findings {
				if item.Code == "recipe_unsupported" || (item.Severity == PreflightBlocked && strings.HasPrefix(item.FieldID, "configuration.build")) {
					t.Fatalf("refused before deploy: %#v", item)
				}
			}
			if fixture.code != "" {
				if item := findingByCode(findings, fixture.code); item == nil || item.Severity != PreflightWarning {
					t.Fatalf("%s = %#v in %#v", fixture.code, item, findings)
				}
			}
		})
	}
...
func TestDetectionMarksCandidatesTheRecipeWouldRefuse(t *testing.T) {
	// No package manager can run a build whose script calls deno, so every
	// install detection recorded is refused, and so is the candidate.
	root := t.TempDir()
	writeBuildFixture(t, root, "package.json", `{"name":"web","scripts":{"build":"deno task build && next build","start":"next start"},"dependencies":{"next":"16.0.0"}}`)
	...
	if candidate := detection.Candidates[0]; !strings.Contains(candidate.RecipeIssue, "deno") ||
		candidate.Confidence != ConfidenceLow {
		t.Fatalf("a build no manager can run read as buildable: %#v", candidate)
	}

...
var nodeManagerNames = map[string]string{"npm": "npm", "bun": "Bun", "pnpm": "pnpm", "yarn": "Yarn"}
...
func buildCauseFix(cause *BuildCause, context causeContext) *CauseFix {
	...
	case "build_lockfile_out_of_sync":
		...
		for _, lockfile := range context.candidate.Lockfiles {
			if lockfile.State == "in_sync" && lockfile.Manager != failing && nodeManagerLockfiles[lockfile.Manager] != "" {
	...
	case "build_command_not_found":
		...
		if rewritten := rewriteNodeRunner(command, manager); rewritten != command {
	...
	case "build_prisma_client_missing":
		...
		exec := map[string]string{"npm": "npx", "bun": "bunx", "pnpm": "pnpm exec", "yarn": "yarn"}[manager]
		return &CauseFix{Kind: fixSetBuild, Field: "configuration.build.buildCommand", Value: exec + " prisma generate && " + command}
...
// rewriteNodeRunner moves the plain package-manager runners in a command to
// manager: `npm run build` becomes `bun run build` on a Bun image. Anything
// that is not exactly a runner is left as it was written.
func rewriteNodeRunner(command, manager string) string {
	run := map[string]string{"npm": "npm run", "bun": "bun run", "pnpm": "pnpm run", "yarn": "yarn run"}[manager]
	exec := map[string]string{"npm": "npx", "bun": "bunx", "pnpm": "pnpm exec", "yarn": "yarn"}[manager]
	if run == "" {
		return command
	}
	...
	for index, segment := range segments {
		...
		rest := fields[1:]
		switch {
		case (fields[0] == "npm" || fields[0] == "bun" || fields[0] == "pnpm" || fields[0] == "yarn") && fields[1] == "run" && len(fields) > 2:
			segments[index] = leading + run + " " + strings.Join(fields[2:], " ") + trailing
		case fields[0] == "npx" || fields[0] == "bunx":
			segments[index] = leading + exec + " " + strings.Join(rest, " ") + trailing
		case fields[0] == "pnpm" && fields[1] == "exec" && len(fields) > 2:
			segments[index] = leading + exec + " " + strings.Join(fields[2:], " ") + trailing
		}
	}
...
func (c *BuildCause) explain() (string, string) {
	...
	case "build_lockfile_out_of_sync":
		...
		if fix != nil && fix.Kind == fixSetBuild {
			return what, "build with " + nodeManagerNames[fix.Value] + ", whose " + nodeManagerLockfiles[fix.Value] +
	...
	case "build_package_manager_mismatch":
		if fix != nil {
			return "package.json's `packageManager` field names " + subject + ", and the build runs another package manager",
				"build with " + nodeManagerNames[subject] + ", the package manager the project declares"
		}
	...
	case "build_prisma_client_missing":
		...
		return "the build used Prisma Client before `prisma generate` created it",
			"run `prisma generate` before the build, for example in the build command or a postinstall script"

...
// ignore file: the repository's patterns, then exceptions for those inputs.
func TestGeneratedDockerignoreKeepsInstallInputs(t *testing.T) {
	...
	if strings.Contains(string(ignore), "!.env") {
		t.Fatal("the operator's own exclusion of .env was undone")
	}
	for _, want := range []string{".dockerignore excludes bun.lock", ".dockerignore excludes .npmrc"} {
	...
	// Without a .dockerignore of its own a repository keeps BuildKit's default.
	plain := writeNodeTree(t, map[string]string{"package.json": leftPad(""), "bun.lock": bunLock})
	if _, err := NewArtifactBuilder(&artifactBackendFake{}).Prepare(context.Background(), plain,
		BuildPlanConfig{Method: BuildRecipe, Recipe: "node", StartCommand: "bun run start"}, false, "t:1"); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(plain, ".just-dashboard", "Dockerfile.dockerignore")); !os.IsNotExist(err) {
		t.Fatalf("an ignore file was written without a repository one: %v", err)
	}
}

...
// unpinnedDependenciesAction is how each recipe's lockfile is committed.
func unpinnedDependenciesAction(recipe string) string {
	lockfile := map[string]string{
		"ruby": "Run `bundle lock` and commit Gemfile.lock", "elixir": "Run `mix deps.get` and commit mix.lock",
		"dart": "Run `dart pub get` and commit pubspec.lock", "gleam": "Run `gleam deps download` and commit manifest.toml",
		"php": "Run `composer update --lock` and commit composer.lock", "rust": "Run `cargo generate-lockfile` and commit Cargo.lock",
		"deno": "Run `deno install` and commit deno.lock",
	}[recipe]
	if lockfile == "" {
		lockfile = "Commit a lockfile (uv lock, poetry lock, or pip freeze > requirements.txt)"
	}
	return lockfile + " when rebuilds must be identical; deploying as is works today."
}

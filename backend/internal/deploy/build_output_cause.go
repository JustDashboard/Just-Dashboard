...
func buildFailureCause(err error, collector *buildOutputCollector, context causeContext, redact func(string) string) *BuildCause {
	...
	if errors.Is(err, dockerx.ErrBuildTimeout) {
		...
		if strings.HasPrefix(instruction, "RUN ") {
			cause.Command = boundedCauseCommand(recipeStepCommand(context, stripRunFlags(command)))
	...
	if failure.Command != "" {
		cause.Command = boundedCauseCommand(recipeStepCommand(context, redact(failure.Command)))
	}
...
var installCommandRE = regexp.MustCompile(`(?:^|&& |; )(?:[A-Z][A-Z0-9_]*=\S* )*(?:npm (?:ci|install)|corepack enable|pnpm install|yarn install|yarn$|bun install|pip3? install|python3? -m pip|uv (?:sync|pip)|poetry install|pipenv install|go mod download|go list -e -deps|cargo fetch|mvn .*dependency:|gradle .*dependencies|\./gradlew .*dependencies|dotnet restore|deno (?:install|cache)|composer install|bundle install|bundle lock --add-platform|mix deps\.get|mix deps\.compile|mix local\.hex|mix assets\.setup|dart pub get|dart pub global activate|gleam deps download|sbt -batch update|lein deps|clojure -P|apk add|apt-get|install-php-extensions)`)
...
// recipeDefaultsRE is the prefix the JavaScript recipe puts before a step
// that needs a value only when the build supplies none — Prisma's env()
// placeholders, SKIP_ENV_VALIDATION, the legacy OpenSSL provider — written as
// `export NAME="${NAME:-…}" … && ` (build_node_runtime.go).
var recipeDefaultsRE = regexp.MustCompile(`^export (?:[A-Za-z_][A-Za-z0-9_]*="[^"]*" ?)+&& `)

// recipeStepCommand is the command a generated step runs, without the
// defaults the recipe exported before it: that is the command its phase and
// its sentence name. A repository's own Dockerfile keeps its words.
func recipeStepCommand(context causeContext, command string) string {
	if context.build.Method != BuildRecipe {
		return command
	}
	return recipeDefaultsRE.ReplaceAllString(strings.TrimSpace(command), "")
}
...
func preparedNodeManager(prepared PreparedBuild) string {
	...
	// Preparation records the install it rendered; a build recorded before
	// it did is read from its Dockerfile.
	if manager := nodeManagerForCommand(prepared.Install); manager != "" {
		return manager
	}

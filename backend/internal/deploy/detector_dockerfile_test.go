...
// Readiness, state and the start command read the stage a Dockerfile
// candidate builds, not a development stage written after it, and each of a
// root's Dockerfiles is read for its own candidate.
func TestDockerfileCandidatesAreReadThroughTheStageTheyBuild(t *testing.T) {
	t.Parallel()
	result := detectFixture(t, map[string]string{
		"Dockerfile": "FROM node:22-alpine AS production\nWORKDIR /app\nCOPY . .\nVOLUME /app/uploads\n" +
			"HEALTHCHECK CMD wget -qO- http://localhost:3000/healthz || exit 1\nEXPOSE 3000\nCMD [\"node\", \"server.js\"]\n" +
			"FROM production AS development\nVOLUME /app/scratch\nHEALTHCHECK NONE\nCMD [\"sh\", \"-c\", \"npm run dev &\"]\n",
		"Dockerfile.worker": "FROM node:22-alpine\nWORKDIR /app\nCOPY . .\nVOLUME /app/queue\nCMD [\"node\", \"worker.js\"]\n",
	...
	for _, candidate := range result.Candidates {
		if candidate.BuildMethod == BuildDockerfile {
			candidates[candidate.Dockerfile] = candidate
		}
	}
	web, worker := candidates["Dockerfile"], candidates["Dockerfile.worker"]
	if web.DockerfileTarget != "production" {
		t.Fatalf("target = %q", web.DockerfileTarget)
	}
	if web.Readiness == nil || web.Readiness.Path != "/healthz" {
		t.Fatalf("readiness = %+v, want the production stage's HEALTHCHECK", web.Readiness)
	}
	if web.StartDetaches != nil {
		t.Fatalf("the development stage's command was read: %+v", web.StartDetaches)
	}
	volumes := func(candidate DetectedCandidate) []string {
		paths := []string{}
		for _, entry := range candidate.PersistentPaths {
			if entry.Kind == PersistentVolume {
				paths = append(paths, entry.Path)
			}
		}
		return paths
	}
	if got := strings.Join(volumes(web), ","); got != "/app/uploads" {
		t.Fatalf("web volumes = %s", got)
	}
	if got := strings.Join(volumes(worker), ","); got != "/app/queue" {
		t.Fatalf("worker volumes = %s", got)
	}
}

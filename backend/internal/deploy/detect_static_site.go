...
// svelteKitFallback is the page adapter-static writes for the paths it did
// not prerender, which is the page nginx falls back to.
func svelteKitFallback(read siteFiles) string {
	for _, extension := range jsConfigExtensions {
		content, ok := read("svelte.config" + extension)
		if !ok {
			continue
		}

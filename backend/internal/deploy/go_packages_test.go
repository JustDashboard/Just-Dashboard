...
func TestGoDetectionAsksForTheMainPackageAndRefusesALibrary(t *testing.T) {
	...
	candidate = detection.Candidates[0]
	if !candidate.GoLibrary || candidate.Confidence != ConfidenceLow || !strings.HasPrefix(candidate.Name, "Go library") {
		t.Fatalf("library module = %#v", candidate)
	}
	if item := findingByCode(plannedRecipeFindings(&candidate, BuildPlanConfig{Method: BuildRecipe, Recipe: "go"}), "go_main_missing"); item == nil ||

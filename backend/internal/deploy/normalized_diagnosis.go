...
// with for the same root and method.
func (e *NormalizedStepExecutor) causeCandidate(ctx context.Context, runID int64, plan *StoredExecutionPlan) *causeCandidate {
	if snapshot, err := e.store.Snapshot(ctx, runID); err == nil {
		...
		if latest != nil {
			var evidence struct {
				Candidate *causeCandidate `json:"candidate"`
			}
	...
	for _, stored := range plan.BuildEvidence.Candidates {
		if stored.BuildMethod != plan.Build.Method || !sameBuildRoot(stored.Root, plan.Build.RootDirectory) {
			continue
		}

...
func applyPlatformTarget(candidate *DetectedCandidate, target platformTarget, marker *detectedMarkers) {
	...
	// own shape for the name.
	for _, name := range manifest.GeneratedVariables {
		for index := range candidate.Variables {
			if variable := &candidate.Variables[index]; variable.Name == name && variable.Setup == "" {
				variable.Setup, variable.SetupReason = "generate", platform+" generates it"
				variable.GenerateFormat, variable.GenerateLength = "hex", 64
			}
		}
	}

		...
		for _, process := range selected.Processes {
			...
			if process.Kind == "release" {
				...
				findings = append(findings, finding("release_process_not_run", PreflightWarning,
					...
					process.Reason+"; this server does not run a release phase from another platform's files.",
					"Chain it in front of the start command (<release> && <start>) if the application needs it before it serves.",

...
func renderRustDockerfile(recipe rustRecipe, config BuildPlanConfig, serving staticServing, bases []ResolvedImage, installSecrets, buildSecrets string) ([]string, error) {
	if len(bases) != 2 {
	...
	case recipe.trunk != "":
		return renderTrunkDockerfile(recipe, config, serving, bases, installSecrets, buildSecrets), nil
	}
	locked := ""
	if recipe.locked {
		locked = " --locked"
...
func renderTrunkDockerfile(recipe rustRecipe, config BuildPlanConfig, serving staticServing, bases []ResolvedImage, installSecrets, buildSecrets string) []string {
	locked := ""
	if recipe.locked {
		locked = " --locked"
	}
	...
	return append(lines, serving.stage(bases[1], "build", "/out/dist/")...)
}

...
func TestSiteOutputsAndFallbacksAreRead(t *testing.T) {
	t.Parallel()
	if fallback := svelteKitFallback(mapSiteFiles(map[string]string{"svelte.config.js": "adapter({ pages: 'build', fallback: '200.html' })"})); fallback != "200.html" {
		t.Fatalf("fallback = %q", fallback)
	}
	if fallback := svelteKitFallback(mapSiteFiles(map[string]string{"svelte.config.js": "adapter({ fallback: '../x.html' })"})); fallback != "" {

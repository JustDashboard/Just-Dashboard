...
var (
	...
	scriptRunRE       = regexp.MustCompile(`\b(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?([A-Za-z0-9:_\-]+)`)
)
...
func localhostExamplePort(example string) (int, bool) {
	...
	if err != nil || (parsed.Scheme != "http" && parsed.Scheme != "https") || !loopbackHost(parsed.Hostname()) || parsed.Port() == "" {
		return 0, false
	}
...
func localhostExamplePath(example string) string {
	parsed, err := url.Parse(example)
	if err != nil || (parsed.Scheme != "http" && parsed.Scheme != "https") || !loopbackHost(parsed.Hostname()) {
		return ""
	}
...
func buildScriptsFetchingLocalhost(stack rootStack) []string {
	...
	for len(queue) > 0 && len(visited) < 32 {
		...
		for _, match := range scriptRunRE.FindAllStringSubmatch(script, -1) {
...
// loopbackHost reports a host that, inside the application's container, is
...
func loopbackValue(name, value string) bool {
	...
	for _, part := range strings.FieldsFunc(value, func(r rune) bool { return r == ';' || r == ' ' }) {
		...
		case "host", "server", "data source", "datasource", "address", "addr", "hostaddr":
			...
			if loopbackHost(host) || strings.EqualFold(host, "(local)") || host == "." {
				return true
			}
		}
	}

...
func TestPreflightJudgesTheBuildsEnvValidation(t *testing.T) {
	...
	// The build runs past the schema, so the environment check does not
	// refuse the names it read from it as build-time values.
	for _, item := range findings {
		if strings.HasPrefix(item.Code, "build_variable_missing_") {
			t.Fatalf("a skipped schema's variable was refused: %+v", item)
		}
	}
	...
	// Without the skip the build stops on these; the environment check
	// refuses each on its own field, so the schema's summary is not repeated.
	findings = preflightFindings(draft, configuration, dockerHost, false)
	for _, name := range []string{"auth_discord_id", "rate"} {
		if item := findingByCode(findings, "build_variable_missing_"+name); item == nil || item.Severity != PreflightBlocked {
			t.Fatalf("%s = %+v", name, item)
		}
	}
	if missing := findingByCode(findings, "build_env_missing"); missing != nil {
...
func TestPreflightNamesPlatformVariablesAnOperatorSet(t *testing.T) {
	...
	for code, measured := range map[string]string{
		...
		"host_variable_loopback_hostname": "HOSTNAME=localhost",
	} {
	...
	findings = preflightFindings(draft, configuration, dockerHost, false)
	for _, code := range []string{"port_variable_mismatch", "node_env_not_production", "host_variable_loopback_hostname", "host_variable_loopback_host"} {

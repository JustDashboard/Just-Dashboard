...
func renderDotnetDockerfile(project dotnetProject, config BuildPlanConfig, serving staticServing, bases []ResolvedImage, installSecrets, buildSecrets string) ([]string, error) {
	want := 2
	if project.spa != nil {
		want = 3
		if project.spa.plan.bun != "" {
	...
	if project.kind == dotnetKindBlazorWasm {
		...
		return append(lines, serving.stage(bases[1], "build", published+"/")...), nil
	}
	lines = append(lines,

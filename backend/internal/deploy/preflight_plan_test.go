...
func TestDetectedVariablesLeftUnsetAreNamedBeforeDeploy(t *testing.T) {
	candidate := &DetectedCandidate{
		Variables: []DetectedVariable{
			...
			{Name: "SENTRY_DSN", Sources: []string{".env.example"}},
			{Name: "LOG_LEVEL", Sources: []string{".env.example"}},
		},
		Databases: []DetectedDatabase{{Engine: "postgres", Variable: "DATABASE_URL"}},
	}
	configuration := PlanConfiguration{Variables: []PlannedVariable{{Name: "LOG_LEVEL"}}}
	findings := detectedVariableFindings(candidate, configuredVariableNames(configuration, map[string]string{"AUTH_SECRET": "staged"}))
	database := findingByCode(findings, "variable_detected_required_database_url")
	if database == nil || database.Severity != PreflightWarning || database.FieldID != "variables.DATABASE_URL" ||
		!strings.Contains(database.Action, "postgres database") || !strings.Contains(database.Measured, "prisma/schema.prisma") {
		t.Fatalf("required variable = %#v", database)
	}
	if findingByCode(findings, "variable_detected_required_auth_secret") != nil {
		t.Fatal("a staged value was reported as missing")
	}
	unset := findingByCode(findings, "variables_detected_unset")
	if unset == nil || unset.Severity != PreflightPass || unset.Measured != "SENTRY_DSN" {
		t.Fatalf("optional remainder = %#v", unset)
	}
	if len(findings) != 2 {

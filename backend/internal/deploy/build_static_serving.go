...
type staticServing struct {
	...
	// site's root: "/index.html", or the page a host's `/* /app.html 200`
	// rule names. page is a framework's own fallback page (SvelteKit's
	// configured "/200.html", React Router's "/__spa-fallback.html", which
	// it writes only when "/" is prerendered), tried as a file before it.
	fallback string
	page     string
...
// recognised framework whose configuration names a base path, and page the
// fallback page it writes into the output the plan serves (nodeServing.page).
func readStaticServing(boundary, root string, config BuildPlanConfig, framework, page string) staticServing {
	...
	if serving.rules.fallback != "" {
		// The host's rule is what the site was served with there.
		serving.fallback, serving.page = serving.rules.fallback, ""
	}
...
func (s staticServing) serverLines() []string {
	...
	if s.spaFallback {
		...
		if s.page != "" && s.page != fallback {
			fallback = s.page + " " + base + fallback
		}

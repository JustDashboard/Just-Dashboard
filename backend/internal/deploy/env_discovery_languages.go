...
var (
	...
	dotnetConnectionNameRE = regexp.MustCompile(`GetConnectionString\(\s*"([A-Za-z][A-Za-z0-9_]*)"\s*\)`)
...
func environmentFactFile(rel, name string) bool {
	switch name {
	case "gemfile.lock", "gemfile", "mix.exs", "pubspec.yaml", "build.sbt", "project.clj", "deps.edn":
		return true
	}
...
func (s *envScanner) scanLanguage(extension, rel, name string, content []byte) {
	...
	case ".cs", ".fs":
		...
		for _, match := range dotnetConnectionNameRE.FindAllSubmatch(content, -1) {
...
// callArgumentList splits a call's remaining arguments, from just after the
// first one up to its closing parenthesis, at their top-level commas —
// skipping nested brackets and string literals — bounded.
func callArgumentList(content []byte, offset int) []string {
	arguments := []string{}
...
func (s *envScanner) scanPython(rel, name string, content []byte) {
	...
	for _, match := range pythonCallReadRE.FindAllSubmatchIndex(content, -1) {
		...
		required := rest == ")"
		if rest == "," {
			required = !pythonReadHasDefault(string(content[match[2]:match[3]]), callArgumentList(content, match[1]+1))
		}
		if required {

...
func dryRunBuild(ctx context.Context, sourceRoot string, build BuildPlanConfig, buildVariableNames []string) error {
	...
	// Within the checkout, as prepare_context prepares it: a workspace
	// member installs from the workspace root above its own directory.
	_, err := builder.PrepareWithin(ctx, sourceRoot, buildRoot, build, false, "just-dashboard/preflight:dry-run", buildVariableNames...)
	return err
}
...
func applyDetectedRecipeIssues(ctx context.Context, root string, candidates []DetectedCandidate) {
	for index := range candidates {
		...
		issue := ""
		if candidate.Recipe == "node" && len(candidate.NodeInstalls) > 0 {
			// The install planner judged every manager by the recipe's own
			// rules when detection recorded them; asking the recipe again
			// would read each lockfile once more per package.
			issue = nodeInstallsRefusal(candidate.NodeInstalls, candidate.PackageManager)
		} else {
			issue = detectedRecipeIssue(ctx, root, *candidate)
		}
		if issue != "" {
			candidate.RecipeIssue = issue
			// A refusal is about building the application, not about what
			// the repository is for, so roots are still compared by the
			// evidence (detect_ranking.go).
...
// nodeInstallsRefusal is the recipe's refusal when no package manager an
// operator can choose would prepare the build — the resolved one's, or the
// first — and empty while any one does.
func nodeInstallsRefusal(installs []DetectedNodeInstall, resolved string) string {
	refusal := ""
	for _, install := range installs {
		blocked := slices.IndexFunc(install.Findings, func(item PreflightFinding) bool { return item.Severity == PreflightBlocked })
		if blocked < 0 {
			return ""
		}
		if refusal == "" || install.Manager == resolved {
			refusal = install.Findings[blocked].Measured
		}
	}
	return refusal
}

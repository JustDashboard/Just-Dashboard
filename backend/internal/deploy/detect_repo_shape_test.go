...
const (
	...
	nextManifest    = `{"name":"web","scripts":{"build":"next build","start":"next start"},"dependencies":{"next":"16","react":"19"}}`
)
...
func TestCandidateRankOrdersTiersConfidenceAreaAndDepth(t *testing.T) {
	...
	for _, fixture := range []struct {
		...
		{"same root ties", candidate("", BuildDockerfile, ConfidenceHigh), candidate("", BuildRecipe, ConfidenceHigh), 0},
	} {
		t.Run(fixture.name, func(t *testing.T) {
			got := compareCandidateRank(fixture.a, fixture.b)
			if (got > 0) != (fixture.want > 0) || (got == 0) != (fixture.want == 0) {
				t.Fatalf("compare = %d, want sign of %d", got, fixture.want)
			}
			if fixture.want != 0 && compareCandidateRank(fixture.b, fixture.a) >= 0 {
				t.Fatal("rank is not antisymmetric")
			}
		})
	}
	tied := []DetectedCandidate{candidate("", BuildDockerfile, ConfidenceHigh), candidate("", BuildRecipe, ConfidenceHigh)}
	if rankedSelection(tied) != "" {

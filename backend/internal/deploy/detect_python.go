...
// with the plan's package manager and Node version.
func pythonAssetInstall(root, dir, selected, nodeVersion, arch string) (nodeInstallSource, nodeInstallPlan, error) {
	...
	plan := planNodeInstall(source.facts, nodeInstallChoice{selected: selected, build: "npm run build", assets: true, nodeVersion: nodeVersion})

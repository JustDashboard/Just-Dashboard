...
// choosePythonBuildVersion is the interpreter a build at a root runs on: the
// plan's setting, else what the source pins, within what its dependencies
// publish wheels for on the build's architecture.
func choosePythonBuildVersion(explicit string, files pythonVersionFiles, project pythonProject, arch string) (pythonVersionChoice, error) {
	inputs := pythonVersionInputsFor(explicit, files, project, arch)
	choice, err := resolvePythonVersion(inputs)
	if err != nil {
		return choice, err
	}
...
func selectPythonRecipe(boundary, root string, config BuildPlanConfig) (pythonRecipe, error) {
	...
	choice, err := choosePythonBuildVersion(config.PythonVersion, versionFiles, memberProject, arch)
	if err != nil {
		return recipe, err
	}
	...
	if dir, ok := pythonAssetsDirectory(root); ok && workspace == nil {
		source, plan, err := pythonAssetInstall(root, dir, config.PackageManager, config.NodeVersion, nodeTargetArch(config.TargetPlatform))
		if err != nil {
			return recipe, err
		}
		if plan.blocked != nil {

...
func TestBlazorWebAssemblyIsPublishedAndServedAsAStaticSite(t *testing.T) {
	...
	assertCompiledDockerfile(t, prepared.DockerfilePreview, "FROM mcr.microsoft.com/dotnet/sdk:9.0@sha256:", "RUN dotnet publish Client.csproj -c Release --no-restore -o /out",
		"RUN test -f /out/wwwroot/index.html", "FROM nginx:1.29-alpine@sha256:", "try_files $uri $uri.html $uri/ /index.html;", "COPY --from=build /out/wwwroot/ /usr/share/nginx/html/")
...
func (b *missingImageBackend) ResolveImage(ctx context.Context, reference, auth string) (ResolvedImage, error) {
	if reference == b.missing {
		return ResolvedImage{}, errors.New("Error response from daemon: manifest unknown: manifest for " + reference + " is not found")
	}

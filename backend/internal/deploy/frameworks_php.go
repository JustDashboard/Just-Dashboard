...
func phpAssetInstallFacts(candidate *DetectedCandidate, marker *detectedMarkers, script string) {
	...
	candidate.NodeInstalls = facts.detectedInstalls(candidate.PackageManager, true, phpAssetStageTools, func(runner string) (string, string) {
...
// phpAssetStageTools are what the asset stage has beside Node, since it
// builds on the vendor stage (phpAssetStage): a package script that runs
// `php artisan` or Composer finds them.
var phpAssetStageTools = []string{"php", "composer"}
...
func phpAssetInstall(root string, config BuildPlanConfig, script string) (nodeInstallSource, nodeInstallPlan, error) {
	...
	plan := planNodeInstall(source.facts, nodeInstallChoice{selected: config.PackageManager, build: "npm run " + script,
		assets: true, provided: phpAssetStageTools, nodeVersion: config.NodeVersion})
...
func selectPHPRecipe(boundary, root string, config BuildPlanConfig) (phpRecipe, error) {
	...
	if assets.kind != "" && !assets.mixUnbuilt && assets.output != "" && phpBuildsAssets(project.wordpress.shape) {
		source, plan, err := phpAssetInstall(root, config, assets.script)
		if err != nil {
			return phpRecipe{}, err
		}
		if plan.blocked != nil {

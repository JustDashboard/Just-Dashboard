...
import { Check, ChevronDown, Linked, Plus } from "@/components/icons"
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className={cn(
            "group flex max-w-full min-w-0 items-center gap-1.5 rounded-md text-left text-2xl leading-tight font-semibold tracking-tight focus-ring",
            className,
          )}
        >
      ...
      <DropdownMenuContent align="start" className="w-80">
        ...
        {connections.map((c) => (
          ...
          >
            ...
            {c.id === current.id && <Check className="mt-0.5 size-3.5 text-foreground" />}
          </DropdownMenuItem>
        ))}
        {(onNew || onConnect) && (
          <>

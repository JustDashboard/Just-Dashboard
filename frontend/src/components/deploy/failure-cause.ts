...
import { PACKAGE_MANAGER_LABELS } from "@/components/deploy/deployment-defaults"
...
export function fixTarget(
  ...
  if (field.startsWith("runtime.")) {
    ...
    if (setting === "mounts") return { href: `${base}/storage`, label: "Review the storage" }
  ...
  switch (setting) {
    case "packageManager":
      return fix.value && fix.value in PACKAGE_MANAGER_LABELS
        ? {
            href: `${base}/build#build`,
            label: `Build with ${PACKAGE_MANAGER_LABELS[fix.value as NodePackageManager]}`,
          }

 ...
 * Node recipe's install — PHP's asset stage, and the Node toolchain the Ruby
 * and Elixir builds borrow — so the package manager choice applies to it
 * while its build and start commands stay the language's own.
 */
export function installsAssetsWithNode(recipe: DeploymentRecipe | undefined) {
  return recipe === "php" || recipe === "ruby" || recipe === "elixir"
}

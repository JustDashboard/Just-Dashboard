...
export function TrafficFacets({
  ...
  return (
    <div className="grid grid-cols-1 gap-x-8 gap-y-6 px-3 py-4 md:grid-cols-2 xl:grid-cols-3">
      {summary.scanners.length > 0 && (
        <Notice tone="warning" title="Scanners" className="md:col-span-2 xl:col-span-3">
      ...
      <Facet title="Pages" eyebrowHint={`${summary.pages.toLocaleString()} page views`}>
        <BarList
      ...
      <Facet title="Clients" eyebrowHint={`${summary.clients.length} shown`}>
        <BarList
      ...
      <Facet
        ...
        eyebrowHint={
          bots + humans > 0
            ? `${Math.round((bots / (bots + humans)) * 100)}% bots`
            : undefined
        }
      >
        <BarList
          items={rank(summary.agents).map((a) => ({
            ...
            label: (
              <>
                {a.value}
                {isBot(a.value) && (
                  <Tag className="ml-2 align-middle">bot</Tag>
                )}
              </>
            ),
        ...
        <BarList
          items={summary.statuses.map((s) => {
            ...
            return {
              ...
              signal: code >= 500 ? 1 : code >= 400 ? 1 : 0,
      ...
      {latencyKnown && (
        <Facet title="Slowest" eyebrowHint="in this window">
              ...
              {slowest.map((e, i) => (
                <li key={`${e.seq ?? i}`}>
                  <button
                    ...
                    className="flex w-full min-w-0 items-baseline gap-3 rounded-sm px-2 py-1 text-left font-mono text-xs focus-ring-inset transition-colors hover:bg-row-hover"
                  >
      ...
      <Facet title="Served" eyebrowHint={bytes(summary.bytes)}>
        <p className="py-1 text-hint text-muted-foreground">
          {bytes(summary.bytes)} sent in {summary.total.toLocaleString()} answers —{" "}
          {summary.total > 0 ? bytes(summary.bytes / summary.total) : "—"} each on average.
        </p>
      </Facet>
    </div>
  )
  ...
  return (
    <section className="min-w-0">
      <div className="mb-2 flex items-baseline justify-between gap-3 border-b border-hairline pb-1.5">
        ...
        {eyebrowHint && (
          <span className={cn("numeric truncate text-hint text-muted-foreground")}>{eyebrowHint}</span>
        )}
      </div>
      {children}
    </section>
